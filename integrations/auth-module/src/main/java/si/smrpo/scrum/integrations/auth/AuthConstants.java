package si.smrpo.scrum.integrations.auth;

public class AuthConstants {
    
    public static final String USERNAME_PARAM = "username";
    public static final String PASSWORD_PARAM = "password";
    public static final String REQUEST_ID_PARAM = "request_id";
    public static final String REFRESH_TOKEN_PARAM = "refresh_token";
    public static final String REDIRECT_URI_PARAM = "redirect_uri";
    public static final String AUTHORIZATION_CODE_PARAM = "code";
    public static final String ERROR_PARAM = "error";
    public static final String SUCCESS_PARAM = "success";
    public static final String GRANT_TYPE_PARAM = "grant_type";
    public static final String PROMPT_PARAM = "prompt";
    public static final String SESSION_STATE_PARAM = "session_state";
    public static final String POST_LOGOUT_REDIRECT_URI_PARAM = "post_logout_redirect_uri";
    public static final String POST_REGISTRATION_REDIRECT_URI_PARAM = "post_registration_redirect_uri";
    public static final String POST_PROFILE_REDIRECT_URI_PARAM = "post_profile_redirect_uri";
    public static final String CODE_2FA_PARAM = "code_2fa";
    public static final String CHALLENGE_2FA_PARAM = "challenge_2fa";
    public static final String CHALLENGE_RESET_PASSWORD_PARAM = "password_reset_code";
    
    
    public static final String CODE_CHALLENGE_METHOD_PARAM = "code_challenge_method";
    public static final String CODE_CHALLENGE_PARAM = "code_challenge";
    public static final String CODE_VERIFIER_PARAM = "code_verifier";
    
    public static final String CONFIRM_PASSWORD_PARAM = "confirm_password";
    public static final String AVATAR_PARAM = "avatar";
    public static final String FIRST_NAME_PARAM = "first_name";
    public static final String LAST_NAME_PARAM = "last_name";
    public static final String EMAIL_PARAM = "email";
    
    public static final String SESSION_INIT_COOKIE = "AUTHZ_SCRUM_SESSION_INIT";
    public static final String SESSION_COOKIE = "AUTHZ_SCRUM_SESSION";
    public static final String REMEMBER_ME_COOKIE = "AUTHZ_SCRUM_REMEMBER_ME";
}
